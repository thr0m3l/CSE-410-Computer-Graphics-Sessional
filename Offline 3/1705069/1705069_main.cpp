...
void display() {
  ...
  ********************/
  ...
  gluLookAt(eye.x, eye.y, eye.z, eye.x + lookVec.x, eye.y + lookVec.y,
            eye.z + lookVec.z, upVec.x, upVec.y, upVec.z);
...
void capture() {
  ...
  double planeDistance =
      (windowHeight / 2.0) / tan((viewAngle / 2.0) * (PI / 180));
  ...
  for (int i = 0; i < imageWidth; i++) {
    for (int j = 0; j < imageHeight; j++) {
      ...
      int nearest = Object::findNearestIntersectingObject(ray, color);

      if (nearest != -1) {
        tMin = objects[nearest]->handleIllumination(ray, color, 1);
      }
      color.clip();
...
void mouseListener(int button, int state, int x,
  ...
  case GLUT_RIGHT_BUTTON:
    if (state == GLUT_DOWN) { // 2 times?? in ONE click? -- solution is checking
                              // DOWN or UP
      drawaxes = 1 - drawaxes;
    }
    break;
...
void loadData() {
  ...
  if (!sceneFile.is_open()) {
    cerr << "Error : Unable to open file scene.txt" << endl;
    exit(1);
  }
  ...
  for (int i = 0; i < light_count; i++) {
    ...
    s = new Light(position, color);
    s->cutoff_angle = cutoff_angle;
    s->is_spotlight = true;
